...
_Snapshot rebuilt in full by `factory-triage` on 2026-09-01, superseding the `factory-spec`
snapshot of 2026-08-31 (gate 4 revision 3). Live labels are the source of truth._

Since that snapshot, FQ-3 closed complete: #38 and #39 (integration slices 4a/4b) merged, and
the parent tracking issue #3 itself closed `completed` on 2026-08-31 — every entry the prior
snapshot carried under "Claimable now" and "Blocked on its own slices" no longer exists as
open work. This run rebuilt the queue from GitHub rather than trusting that stale shape.

13 open issues total, 0 skipped (cap is 20). 1 issue was untriaged (#32, no state label). 11 of
the remaining 12 had changed since the prior triage run
(`docs/factory/runs/2026-08-31T060646Z-triage-run.md`, finished `2026-08-31T06:08:15Z`) and were
re-examined; #11 and #12 last changed before that run finished and were not re-examined.
...
**The review queue is empty.** #46 may run alone; nothing else is claimable until it lands.
...
- load_bearing: true
- gate_level: deep
- done_when: `./.claude/scripts/gates.sh full` prints a FACTORY_GATES line carrying `base_source=`, `protected_globs=` and `protected=`; `gates.sh fast` prints `base_source=not-measured protected=not-measured`; a bash older than 3.2 prints `misconfigured=unsupported-bash` and exits 2; the three tests pass, including one that runs the real `./.claude/scripts/gates.sh fast` in this repository and parses its line; no pre-existing test file is modified; gates GREEN at `deep`
- confidence: medium
- notes: Slice 0 of FQ-23, gates 1–4 approved by a human in session on 2026-08-31. Confirmed unchanged this run — label and handoff comment (2026-08-31T21:36:39Z) are current. `.claude/scripts/gates.sh` is Charter §2 LOAD_BEARING and `Edit`-denied in `.claude/settings.json`; the implement run will stop and need a human despite the `ready-to-implement` label, and the PR needs a human read regardless of gate colour. #47, #49 and #50 are each blocked on this slice; #48 is blocked on #47.
...
- confidence: high
- notes: Re-derived this run, not carried forward. A `factory-spec` run took this through all four gates since the prior triage (approved 2026-08-31) and filed slices #46–#51. Disposition moved from `ready-to-spec` to `wait-to-implement`; handoff comment updated to supersede the 2026-08-30 one.
...
- confidence: high
- notes: Blocked on #46 (needs the field plumbing and `resolve_base` slice 0 adds). Label and handoff comment (2026-08-31T21:36:41Z) confirmed current this run.
...
- confidence: high
- notes: Blocked on #47 (refines `measure_protected`, which slice 1 introduces). Label and handoff comment (2026-08-31T21:36:42Z) confirmed current this run.
...
- confidence: high
- notes: Blocked on #47 (completes `parse_charter_section3`). Label and handoff comment (2026-08-31T21:36:44Z) confirmed current this run.
...
- confidence: high
- notes: Blocked on #46 (`resolve_base`'s `$2` branch is this slice's addition to slice 0's function). Label and handoff comment (2026-08-31T21:36:45Z) confirmed current this run.
...
- confidence: medium
- notes: Blocked on #46 (`resolve_base` and field plumbing). Deliberately droppable — gate 1 approved the §7 ceiling as independently reversible from the §3 hole slices 0–4 close. Label and handoff comment (2026-08-31T21:36:46Z) confirmed current this run.
...
- last_triaged: 2026-08-31 (not re-examined this run — unchanged since prior triage)
...
- gate_level: deep
- confidence: medium
- notes: Previously untriaged (no state label). Splits into a docs-only correction (Charter §4 AUTOMATABLE) and a skill-minting-rule change under `.claude/skills/**` (Charter §2 LOAD_BEARING, forcing NEEDS_SPEC). Filed as one issue covering both, so the split itself is the scope decision `ready-to-spec` exists for.
...
- gate_level: deep
- confidence: high
- notes: Confirmed against the charter (previously self-labeled by the filing session; this run re-derived independently). Touches skill files that compose PR bodies — Charter §2 LOAD_BEARING. Worth prioritizing: review queue is empty, and the defect has already cost two queue items.
...
- gate_level: full
- confidence: high
- notes: Confirmed against the charter (previously self-labeled by the filing session; this run re-derived independently). Changes a public MCP read surface — Charter §4 NEEDS_SPEC.
...
- gate_level: deep
- confidence: high
- notes: Confirmed against the charter (previously self-labeled by the filing session; this run re-derived independently). Touches the contract's claim protocol and `.claude/skills/factory-implement/**` — Charter §2 LOAD_BEARING. Not urgent (nothing broken on `main` today), but real.
